...
import {ArrowLeft, ChevronLeft, ChevronRight, Gem, Search, Shield, Sword, X} from "lucide-react";

const grades = ["All rarities", "Common", "Rare", "Epic", "Unique", "Heroic", "Special", "Mythic"];
const categories = [
  {id: "Weapons", name: "Weapons", icon: Sword, art: "/equipment-art/weapon.webp"},
  {id: "Armor", name: "Armor", icon: Shield, art: "/equipment-art/armor.webp"},
  {id: "Accessories", name: "Accessories", icon: Gem, art: "/equipment-art/accessory.webp"},
];
...
function EquipmentModal({item, region, onClose}) {
  ...
  useEffect(() => {
    ...
    setDetails(null);
    setState("loading");
    fetch(`/api/equipment-encyclopedia?region=${region}&item=${item.id}`, {signal: controller.signal})
      .then(async (response) => {
  ...
  return <div className="equipmentModalBackdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="equipmentModal" role="dialog" aria-modal="true" aria-labelledby="equipmentModalTitle">
      ...
      <div className="equipmentModalTitleRow">
        <img src={item.icon || categories.find(({id}) => id === item.category)?.art || categories[1].art} alt="" />
...
export default function EquipmentBrowser() {
  ...
  useEffect(() => {
    const controller = new AbortController();
    setState("loading");
    const query = new URLSearchParams({region, category, page: String(page)});
    if (grade !== "All rarities") query.set("grade", grade);
    if (submittedSearch) query.set("q", submittedSearch);
    fetch(`/api/equipment-encyclopedia?${query}`, {signal: controller.signal})
      .then(async (response) => {
        const data = await response.json();
    ...
    return () => controller.abort();
  }, [region, category, grade, page, submittedSearch]);
  ...
  return <main className="classPage equipmentBrowsePage generalEquipmentPage">
    ...
    <section className="equipmentGeneralContent">
      <header className="equipmentGeneralHeading">
        ...
        <h1>Equipment</h1>
        <p>Search weapons, armor and accessories. Open any item to see its recorded stats and details.</p>
      </header>
      ...
      <div className="equipmentGeneralTypes" role="tablist" aria-label="Equipment type">
        {categories.map(({id, name, icon: Icon}) => <button type="button" role="tab" aria-selected={category === id} className={category === id ? "isSelected" : ""} key={id} onClick={() => changeFilter(() => setCategory(id))}><Icon aria-hidden="true" />{name}</button>)}
      </div>
      ...
      <div className="equipmentResultsHeader"><span>{submittedSearch ? `Results for “${submittedSearch}”` : category} <b>{pageInfo.total.toLocaleString()}</b></span><span>{region === "KR_TW" ? "KR / TW REFERENCE" : "GLOBAL REFERENCE"}</span></div>
      ...
      {state === "ready" && items.length > 0 && <div className="equipmentItemGrid generalEquipmentGrid">
        {items.map((item) => <button className="equipmentItemCard" type="button" key={`${region}-${item.id}`} onClick={() => setSelectedItem(item)}>
          <span className="equipmentItemIcon"><img src={item.icon || categories.find(({id}) => id === category)?.art} alt="" loading="lazy" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = categories.find(({id}) => id === category)?.art || categories[1].art; }} /></span>
      ...
      {state === "ready" && items.length === 0 && <p className="equipmentLoading">No items match these filters.</p>}
