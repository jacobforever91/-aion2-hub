    ...
    <div className={styles.wrap}><Link className={styles.back} href="/?menu=open" aria-label="Back to menu"><ArrowLeft/></Link>
      ...
      <div className={styles.layout}>
        <section className={styles.catalog}>
          ...
          <div className={styles.list}>{visible.map(item=><button id={`wing-${item.id}`} key={item.id} className={`${styles.item} ${styles.tierItem} ${selected.id===item.id?styles.selected:""}`} style={{"--tier-color":tierColors[item.grade],"--icon-x":`${item.iconPosition[0]*-72}px`,"--icon-y":`${item.iconPosition[1]*-72}px`}} onClick={()=>setSelected(item)}><span className={`${styles.itemIcon} ${styles.artIcon} ${styles.wingArtIcon}`} aria-hidden="true"/><span className={styles.itemCopy}><strong>{item.name}</strong><small>{item.faction}</small></span><span className={styles.tierBadge}>{item.grade}</span>{item.enhancementCap&&<span className={styles.levelPill}>+{item.enhancementCap}</span>}</button>)}</div>
          {!visible.length&&<p className={styles.empty}>No wings match this search.</p>}
        </section>
        <aside className={styles.detail}><div className={styles.detailHeading}><span className={`${styles.detailIcon} ${styles.artIcon} ${styles.wingArtIcon}`} style={{"--tier-color":tierColors[selected.grade],"--icon-x":`${selected.iconPosition[0]*-72}px`,"--icon-y":`${selected.iconPosition[1]*-72}px`}} aria-hidden="true"/><div><span className={styles.kicker}>SELECTED WING</span><h2>{selected.name}</h2></div></div><p className={styles.meta}><span className={styles.tierBadge} style={{"--tier-color":tierColors[selected.grade]}}>{selected.grade}</span> · {selected.faction}{selected.enhancementCap?` · Enchantable to +${selected.enhancementCap}`:""}</p>
